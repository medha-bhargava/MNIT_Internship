...
export const approveStudent = async (req, res) => {
  ...
  try {
    ...
    // You can generate a password or use a default one
    ...
    const newUser = new User({
      ...
      email: student.email,
      role: "student",
      password: generatedPassword, // hash later ideally
    });

    await newUser.save();
    await NewStudent.findByIdAndDelete(studentId);

    res.status(201).json({ message: "Student approved and account created.", password: generatedPassword });
  } catch (err) {
